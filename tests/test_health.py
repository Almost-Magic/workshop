...
def test_health_returns_200(client):
    """Smoke: /workshop/api/health returns HTTP 200."""
    resp = client.get("/workshop/api/health")
    assert resp.status_code == 200


def test_health_refresh_returns_200(client):
    """Smoke: POST /workshop/api/health/refresh returns 200."""
    resp = client.post("/workshop/api/health/refresh")
    assert resp.status_code == 200
...
def test_health_contains_required_fields(client):
    ...
    data = client.get("/workshop/api/health").get_json()
